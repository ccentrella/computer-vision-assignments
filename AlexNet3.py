...
# File location
file_path = f'D:\\Keras\\Logs\\AlexNet3\\{time.strftime("%Y-%m-%d-%H%M%S")}\\'
...
#   C1
model.add(Conv2D(filters=96, kernel_size=3, strides=(1,1), padding='same',
...
#   C3
model.add(Conv2D(filters=384, kernel_size=3, padding='same', kernel_regularizer=L2(0.0005)))
model.add(Activation(ReLU()))
...
earlystopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=15)
...
callbacks=[checkpointer, earlystopping, reduce_lr], validation_data=(x_test, y_test))
